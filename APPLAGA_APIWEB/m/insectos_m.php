...
class insectos_m implements modelo
{
    ...
    public function __construct($id)
    {
        ...
        $sp = $this->bd->store_procedure('obtener_punto_insectos', [$this->id]);
    ...
    public function punto_html()
    {
        if ($this->existe) {
            $cambios = [
                ...
                '{PUNTOCONTROL_MODELO}' => trim($this->datos->PUNTOCONTROL_MODELO),
                '{PUNTOCONTROL_GEOLOCALIZACION}' => $this->datos->PUNTOCONTROL_GEOLOCALIZACION
            ];
            $plantilla_ver = self::FOLDER . '/v/' . self::DEFULT_THEME . '/html/panel_insectos_consumos.html';

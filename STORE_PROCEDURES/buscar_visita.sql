CREATE PROCEDURE `buscar_visita`(
	IN p_clienteid INT,
    IN p_localid INT,
    IN p_finicio DATE,
    IN p_ffinal DATE
...
BEGIN
    ...
    IF p_clienteid = 0 THEN
		...
		WHERE v.inicio BETWEEN p_finicio AND p_ffinal
        AND v.activo = 1
        ORDER BY v.id DESC;
    ELSE
		IF p_localid = 0 THEN
            ...
            LEFT JOIN cliente_local l ON (v.local_id = l.id)
            LEFT JOIN cliente c ON (c.id = l.cliente_id)
            WHERE v.inicio BETWEEN p_finicio AND p_ffinal AND 
            l.cliente_id = p_clienteid AND v.activo = 1
			ORDER BY v.id DESC;
        ELSE
			SELECT '1' as 'resultado', 'ok' as 'mensaje',
				v.id as 'VISITA_ID',
